...
Shader "Water/Fog/NatuarlLitTextureNewFoam" 
{
	/* 
	...
	Properties 
	{
		...
		_FoamDistance ("Foam distance", Range(0.001,2)) = 0.1
	}
	SubShader 
	{
		// Subshader Tags
		...
		////// Shader functions
			...
			{				
				...
				// Foam
				float foam = GetFoamNew(_FoamTex, fragIn.uv_Environment.zw, waterLevel, _FoamWidth, _FoamDistance);
				...
				final.w = max(max(max(depthLevel, specularLight.x),_ShoreEdgeIndicator),_FoamFade-waterLevel);
